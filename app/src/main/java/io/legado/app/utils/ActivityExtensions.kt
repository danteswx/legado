...
@Suppress("DEPRECATION")
fun Activity.setStatusBarHiddenCompat(hidden: Boolean) {
    val decorView = window.decorView
...
@Suppress("DEPRECATION")
fun Activity.setNavigationBarHiddenCompat(hidden: Boolean) {
    val decorView = window.decorView
    val controller = WindowCompat.getInsetsController(window, decorView)
    if (hidden) {
        ...
        decorView.systemUiVisibility =
            decorView.systemUiVisibility and View.SYSTEM_UI_FLAG_HIDE_NAVIGATION.inv()
    }
}
