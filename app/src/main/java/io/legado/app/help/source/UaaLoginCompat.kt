...
object UaaLoginCompat {
    ...
    val LOGIN_UI: String = """
        <js>
        var captcha = "";
        try { captcha = refreshCaptchaValue(false); } catch (e) { java.log("UAA captcha init failed: " + e); }
        JSON.stringify([
        {name:"账号",type:"text"},
        {name:"密码",type:"password"},
        {name:"验证码",type:"text",default:captcha},
        {name:"AI识别验证码",type:"button",action:"refreshCaptcha()",style:{layout_flexGrow:1,layout_flexBasisPercent:0.45}},
        {name:"网页登录兜底",type:"button",action:"openWebLogin()",style:{layout_flexGrow:1,layout_flexBasisPercent:0.45}}
        ]);
        </js>
    """.trimIndent()
    ...
    val LOGIN_URL: String = """
        ...
        function refreshCaptchaValue(up){var u=cap(page());if(!u){if(up)java.toast("未找到UAA验证码");return ""}try{var c=java.aiCaptcha(u,"识别UAA登录验证码，只返回字母和数字，不要解释");if(up)java.upLoginData({"验证码":c});return c}catch(e){java.toast("AI验证码失败: "+e);return ""}}
        function refreshCaptcha(){refreshCaptchaValue(true)}
    ...
    private fun shouldReplaceLoginUrl(loginUrl: String?): Boolean {
        ...
        return !text.contains("function login") || !text.contains("source.putLoginHeader")
    }
}
